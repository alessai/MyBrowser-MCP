...
`new_tab` creates a session-owned temporary tab by default. Pass `temporary: false` when it should persist, or call `keep_tab` before cleanup. After browser research, call `browser_cleanup` on both success and failure paths. It closes tracked temporary tabs on currently connected browsers, releases the caller's tab claims, and restores default routing. Tabs that cannot be closed remain tracked for later cleanup; if the live session opens another tab first, they wait for the next explicit cleanup or session finalization so a stale retry cannot close newer work. Manual, pre-existing, popup-created, non-temporary, kept, and other-session tabs are never inferred as owned and are not closed.
