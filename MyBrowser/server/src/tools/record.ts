...
import {
  SERVER_RECORDING_ARGUMENT_TYPES as PURE_SERVER_RECORDING_ARGUMENT_TYPES,
  SERVER_RECORDING_NUMERIC_BOUNDS as PURE_SERVER_RECORDING_NUMERIC_BOUNDS,
  SERVER_RECORDING_STRING_METADATA as PURE_SERVER_RECORDING_STRING_METADATA,
  type RecordingArgumentType,
  type RecordingNumericConstraint,
  type RecordingStringKind,
  type VariableSource,
} from "./recording-classification.js";
export type { RecordingNumericConstraint } from "./recording-classification.js";
...
const RECORDING_FILE_OPS: RecordingFileOps = {
  noFollowFlag: typeof fsConstants.O_NOFOLLOW === "number"
...
export const SERVER_RECORDING_STRING_METADATA = PURE_SERVER_RECORDING_STRING_METADATA;

export const SERVER_RECORDING_ARGUMENT_TYPES = PURE_SERVER_RECORDING_ARGUMENT_TYPES;

export const SERVER_RECORDING_NUMERIC_BOUNDS = PURE_SERVER_RECORDING_NUMERIC_BOUNDS;
...
export function requirePositiveNoFollowFlag(flag: number | undefined): number {
  if (!Number.isInteger(flag) || (flag ?? 0) <= 0) {
    throw new Error("RECORDING_PERSISTENCE_NOFOLLOW_UNAVAILABLE");
  }
  return flag!;
}
...
function verifyExistingRecording(
  ...
  const noFollow = requirePositiveNoFollowFlag(ops.noFollowFlag);
